...
class Augmentor:
    ...
    def extract_rule(self, rule):
        ...
        rule = {"variable": variable, "type": variableType, "rule": ruleDef}
        return rule
    ...
    def generate_templates(self):
        ...
        while i < len(linelist):
            ...
            if linelist[i][0:6] == "@param":
                ...
                i = i + endindex - 1
                randomizer = {"name": flowname, "rules": rules, "flow": flow}
    ...
    def get_data(self, template):
        ...
        for rule in template["rules"]:
            ...
            ruleOptions = json.loads(
                rule["rule"][
                    rule["rule"].find("[") : rule["rule"].find("]") + 1
                ].strip()
            )
            if ruleType == "choice":
                ...
                flow = flow.replace(
                    rule["variable"],
                    str(random.randint(ruleOptions[0], ruleOptions[1])),
                )
        return flow
